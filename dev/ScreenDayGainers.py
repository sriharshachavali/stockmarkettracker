...
def get_top_penny_stocks():
    ...
    results = screener.get_screeners(['most_actives', 'day_gainers'], count = 1000)
    ...
    df = pd.DataFrame(stocks)
    #Filter for penny stocks (price <=5) and sort by volume and change percentageß
    # df = df[(df['regularMarketPrice']>=1) & (df['regularMarketPrice'] <= 25) & (df['regularMarketVolume'] >=1000000)]
    df = df[df['symbol']=='UAMY']
